package com.huddey.userman.security;
...
class MobileTokenGenerationStrategyTest {
  ...
  @InjectMocks private MobileTokenGenerationStrategy tokenStrategy;
  ...
  void setUp() {
    ...
    User user =
        User.builder().id(1L).email("test@example.com").roles(new HashSet<>(List.of(role))).build();
