...
class WebTokenGenerationStrategyTest {
    ...
    @InjectMocks
    private WebTokenGenerationStrategy tokenStrategy;
    ...
    private static final long REMEMBER_ME_REFRESH_TOKEN_VALIDITY = 2592000000L; // 30 days in milliseconds
    ...
    void generateAndSetToken_WithoutRememberMe_ShouldSetCorrectCookies() {
        ...
        when(jwtTokenProvider.getAccessTokenValidity())
                .thenReturn(ACCESS_TOKEN_VALIDITY);
        when(jwtTokenProvider.getRefreshTokenValidity())
                .thenReturn(REFRESH_TOKEN_VALIDITY);
