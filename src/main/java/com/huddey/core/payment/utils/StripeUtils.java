...
public class StripeUtils {
  ...
  public static Optional<UserSubscription> findUserSubscription(
      String stripeSubscriptionId, UserSubscriptionRepository repository) {
    ...
    try {
      ...
      if (userSub.isEmpty()) {
        log.debug(
            "No subscription found in database for Stripe subscription ID: {}",
  ...
  /**
   * Alternative method to find subscription by customer ID if invoice doesn't contain subscription
   * directly
   */
  public static Optional<UserSubscription> findUserSubscriptionByCustomerId(
      String customerId, UserSubscriptionRepository repository) {
